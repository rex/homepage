...
import offTheClockYaml from '../content/off-the-clock.yaml?raw';
import animationLabYaml from '../content/animation-lab.yaml?raw';
...
export interface OffTheClockContent {
  intro: string;
  certified: string[];
  volunteer: {
    org: string;
...
export type AnimationLabOptionKey = 'ambient' | 'scroll' | 'signal' | 'hover' | 'type' | 'traces';

export interface AnimationLabContent {
  title: string;
  description: string;
  controls_label: string;
  options: {
    key: AnimationLabOptionKey;
    label: string;
    description: string;
...
const offTheClockData = yaml.parse(offTheClockYaml) as OffTheClockContent;
const animationLabData = yaml.parse(animationLabYaml) as AnimationLabContent;
...
export const loadOffTheClock = async (): Promise<OffTheClockContent> => offTheClockData;
export const loadAnimationLab = async (): Promise<AnimationLabContent> => animationLabData;
